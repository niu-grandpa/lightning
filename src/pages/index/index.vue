...
import { ref, nextTick, onBeforeUnmount, onBeforeMount, computed, DefineComponent } from 'vue';
import TabbarItem from '../../assets/tabbar-item';
import { HomeLogo } from '../../components';

type AsyncCompsType = { [x: string]: DefineComponent };
...
<template>
  ...
  <template v-if="!showIndexImage">
    <Suspense>
      <component :is="asyncComps[currentName]" />
    </Suspense>
  </template>

  <nut-tabbar v-model:visible="currentName" bottom safeAreaInsetBottom active-color="#1989fa">
    <nut-tabbar-item
